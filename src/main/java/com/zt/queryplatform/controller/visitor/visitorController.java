...
public class visitorController {
     ...
     * @return
     */
    @GetMapping("/visitor/getBookList/{code}")
    @ResponseBody
    public  ApiResponse getBookList(@RequestParam(name = "start",defaultValue = "0")int start,
        ...
        if(StringUtils.isEmpty(code)){
            libraryId = orginazationService.findOrgIdByCode(code);
        }
        if(libraryId < 0){
...
//        //查询书
//        ServiceMultiResult<BookDTO> bookList = bookService.queryBookList(book, pageable);
//
//        if(bookList.getResultSize() == 0){
//            return ApiResponse.ofSuccess(ApiResponse.Status.NOT_VALID_PARAM);
//        }
...
//         return   ApiResponse.ofMessage(ApiResponse.Status.FORBIDDEN.getCode(),"读取信息错误，无权限操作，请联系管理员！");
//        }
//        ServiceMultiResult<BookDTO> recommendedBookList = recommService.findRecommendedBookList(libraryId);
...
//            return   ApiResponse.ofMessage(ApiResponse.Status.FORBIDDEN.getCode(),"读取信息错误，无权限操作，请联系管理员！");
//        }
//        ServiceMultiResult<BookDTO> rankingListOfBook = lendService.getRankingListOfBook(libraryId);
//
...
//    @GetMapping("/visitor/rankingListOfReader")
//    @ResponseBody
//    public  ApiResponse rankingList(){
//        if(libraryId <= 0){
//            return   ApiResponse.ofMessage(ApiResponse.Status.FORBIDDEN.getCode(),"读取信息错误，无权限操作，请联系管理员！");
//        }
//        Map<String, ServiceMultiResult<LendDTO>> rankingMap = lendService.getRankingList(libraryId);
//
//        if(rankingMap.get("reader").getResultSize() == 0 && rankingMap.get("book").getResultSize() == 0){
//            return ApiResponse.ofMessage(ApiResponse.Status.NOT_VALID_PARAM.getCode(),"无相关数据");
//        }
//        return ApiResponse.ofSuccess(rankingMap);
//    }
